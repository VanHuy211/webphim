...
namespace MovieWeb.Areas.Admin.Controllers
{
    public class DSPhimController : BaseController
    {
        ...
        {
            DSPhimBo tk = data.DSPhimBoes.SingleOrDefault(n => n.ID == id);
            if (tk == null)
            {
                Response.SubStatusCode = 404;
                return null;
            }
            data.DSPhimBoes.Remove(tk);
            data.SaveChanges();
            return RedirectToAction("DSPhimBo");
        }
        //Sửa Phim
        ...
        {
            DSPhimBo phim = data.DSPhimBoes.SingleOrDefault(n => n.ID == id);
        ...
        {
            var phim = data.DSPhimBoes.First(n => n.ID == id);
        ...
        //Xóa tập phim
        public ActionResult XoaTapPhim(int id, int tapphim)
        {
            CTTapPhim tk = data.CTTapPhims.SingleOrDefault(n => n.ID == id && n.TapPhim == tapphim);
            if (tk == null)
            {
                Response.SubStatusCode = 404;
                return null;
            }
            data.CTTapPhims.Remove(tk);
            data.SaveChanges();
        ...
        {
            DSPhimBo phimbo = data.DSPhimBoes.SingleOrDefault(m => m.ID == id);
