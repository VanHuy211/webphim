...
namespace MovieWeb.Areas.Admin.Controllers
{
    public class DanhMucController : BaseController
    {
        ...
        public ActionResult SuaBanner(int id)
        {
            ...
            ViewData["IDPhim"] = new SelectList(data.DSPhimBoes, "ID", "TenPhim", banner.IDPhim);
            return View(banner);
        }
        ...
        {
            var tin = data.gioithieux.First(n => n.idgioitin == id);
            if (tin == null)
            {
                Response.SubStatusCode = 404;
                return null;
        ...
        public ActionResult SuaGioiThieu(int id, FormCollection collection)
        {
            ...
            UpdateModel(tin);
            data.SaveChanges();
            return RedirectToAction("GioiThieuWebXemPhim");
        }
